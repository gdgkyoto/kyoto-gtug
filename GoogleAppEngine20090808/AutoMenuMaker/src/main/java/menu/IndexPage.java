...
public class IndexPage extends WebPage {
	...
	private void createFormPane(final User user) {
		add(new StatelessForm<Void>("form") {
			...
			protected void onSubmit() {
				Entity entity = EntityLogic.create(user,
						contentTextField.getModelObject());
				new EntityLogic().save(entity);
				removeCache();
	...
	private void createListPane() {
		...
		PageableListView<Entity> listView = new PageableListView<Entity>(
				"list", entities, 10) {
			@Override
	...
	private List<Entity> getEntities() {
		String key = Entity.class.getName();
		Cache cache = null;
		try {
			cache = CacheManager.getInstance().getCacheFactory().createCache(
					Collections.emptyMap());
			if (cache.containsKey(key)) {
				return (List<Entity>) cache.get(key);
			}
		...
		List<Entity> list = logic.list(query);
		if (cache != null) {
			cache.put(key, list);
		}
		return list;
